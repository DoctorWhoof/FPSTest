...
Class Graph
	...
	Method New( name:String, color:Color, range:Float )
		Self.color = color
		Self.range = range
		all.Add( name, Self )
	End
	...
	Method Draw( canvas:Canvas, scale:Float, leftBorder:Int, rightBorder:Int )
	...
	Function Add( name:String, value:Double, range:Float, color:Color = Color.White )
		Local g := all[ name ]
		If Not g
			New Graph( name, color, range )
		Else
			g.AddValue( value )
		End
	End
	...
	Function DrawAll( canvas:Canvas, height:Double, leftBorder:Int = 10, rightBorder:Int = 10 )
...
'		canvas.DrawText( -range, leftBorder, bottom, 1, 1 )
		...
		For Local name := Eachin all.Keys
			...
			If g
				...
				g.Draw( canvas, height/g.range, leftBorder, rightBorder )
